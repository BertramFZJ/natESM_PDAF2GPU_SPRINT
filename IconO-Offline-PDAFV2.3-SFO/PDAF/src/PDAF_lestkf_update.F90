...
! *** local variables ***
  ...
  DOUBLE PRECISION :: rseTimerStartThread9, rseTimerFinishThread9, rseTimerLocalThread9, rseTimerTotalThread9(0:255)
  DOUBLE PRECISION :: rseTimerStartThread7, rseTimerFinishThread7, rseTimerLocalThread7, rseTimerTotalThread7(0:255)
  INTEGER :: rseIterCountThreadLocal, rseIterCountThread(0:255)

  INTEGER :: IERROR, iLOOP
  INTEGER :: MPIRANKWORLD, OMPNUMTHREADS
...
! *************************************
  ...
  IF (incremental < 2) THEN
     ! Do prepoststep only if LESTKF is not used in hybrid 3D-Var (incremerseTimerStartThread = MPI_WTIME()ntal==2)
...
! *** Local analysis: initialize global quantities ***
  ...
  WRITE(*,*) 'NATESM LOCAL FULL OBS: ', dim_obs_f
...
! ************************************
  ...
  rseTimerStart = MPI_WTIME()  
  
!$OMP PARALLEL SHARED(OMPNUMTHREADS)
!$OMP MASTER
...
!$OMP BARRIER
    ...
    rseTimerStartThread = MPI_WTIME()
    rseTimerLocalThread7 = 0.0d0
    rseTimerLocalThread9 = 0.0d0
    rseIterCountThreadLocal = 0
...
!$OMP DO firstprivate(cnt_maxlag) lastprivate(cnt_maxlag) schedule(runtime)
  ...
  rseTimerFinishThread = MPI_WTIME()
   rseTimerTotalThread(omp_get_thread_num()) = rseTimerFinishThread - rseTimerStartThread
  rseTimerTotalThread7(omp_get_thread_num()) = rseTimerLocalThread7
...
!$OMP END PARALLEL
  ...
  ! ######################################################################################################################################################
  rseTimerFinish = MPI_WTIME()
  rseTimerTotal = rseTimerFinish - rseTimerStart
  WRITE(*,'(1x, a, F7.2, 1x, I8, 1x, F7.2, 1x, F7.2)') 'NATESM TIMER TOTAL: ', rseTimerTotal, n_domains_p, MINVAL(rseTimerTotalThread(0:OMPNUMTHREADS - 1)), MAXVAL(rseTimerTotalThread(0:OMPNUMTHREADS - 1))
  CALL MPI_ALLREDUCE(rseTimerTotal, rseTimerMin, 1, MPI_DOUBLE_PRECISION, MPI_MIN, MPI_COMM_WORLD, IERROR)
  CALL MPI_ALLREDUCE(rseTimerTotal, rseTimerMax, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_WORLD, IERROR)
  CALL MPI_COMM_RANK(MPI_COMM_WORLD, MPIRANKWORLD, IERROR)
  IF(MPIRANKWORLD .EQ. 0) THEN
    WRITE(*,'(1x, a, F7.2, 3x, F7.2)') 'NATESM TIMER GLOBAL WORLD: ', rseTimerMin, rseTimerMax
  ENDIF
