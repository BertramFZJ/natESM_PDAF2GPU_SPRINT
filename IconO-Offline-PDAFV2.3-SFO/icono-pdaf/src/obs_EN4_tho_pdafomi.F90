...
MODULE obs_EN4_tho_pdafomi
  ...
  LOGICAL :: assim_EN4_tho = .FALSE. !< Whether to assimilate this data type
  REAL    :: rms_obs_EN4_tho         !< Observation error standard deviation (for constant errors)
...
!!
  SUBROUTINE init_dim_obs_EN4_tho(step, dim_obs)
...
! *********************************************
    ...
    IF (mype_filter==0) &
         WRITE (*,'(8x,a)') 'Assimilate observations - EN4_tho'

    ! Store whether to assimilate this observation type (used in routines below)
    IF (assim_EN4_tho) thisobs%doassim = 1
...
! *************************
    ...
    CALL nf_check(NF_INQ_VARID(ncid_in, sfields(id%tho)%cvar, id_state))
    IF (rmsfileflag == 1) THEN
       CALL nf_check(NF_INQ_VARID(ncid_in, sfields(id%tho)%crms, id_rms))
    END IF
    ...
    !monitor
    IF (debug) WRITE(*,*) 'Read complete for '//TRIM(sfields(id%tho)%cvar)// &
    ...
    IF (rmsfileflag == 1) THEN                    ! read from file if flag is set
       ...
       !monitor
       WRITE(*,*) 'Read complete for '//TRIM(sfields(id%tho)%crms)// &
    ...
    ELSE        
       ...
       IF (mype_filter==0) &
            WRITE(*,'(8x,a,es10.3)') 'rms_obs is fixed with rms_obs_EN4_tho:', rms_obs_EN4_tho

       rms_xyz(:,:) = rms_obs_EN4_tho
...
! ***********************************************************
    ...
    IF (npes_filter==1) THEN
       WRITE (*,'(8x, a, i7)') '--- number of observations from EN4_tho: ', dim_obs_p
    ELSE
       IF (screen>2) THEN
          WRITE (*,'(8x, a, i4, 2x, a, i7)') 'PE', mype_filter, &
               '--- number of observations from EN4_tho: ', dim_obs_p
       END IF
    END IF
    ...
    IF (dim_obs_p>0) THEN
       ...
       DO iz = 1, dimz
          DO ixy = 1, dimxy_p
             ...
             IF (obs_xyz(ixy,iz) > -1e33) THEN
                ...
                thisobs%id_obs_p(1, obsrec) = idx_wet_in_all(rec) + sfields(id%tho)%off
       ...
       IF (irms>0) THEN
          WRITE(*,*) TRIM(sfields(id%tho)%crms),' < rms_min (',rms_min,'):', irms
       END IF
...
! ********************
  ...
  END SUBROUTINE init_dim_obs_EN4_tho
...
!!
  SUBROUTINE obs_op_EN4_tho(dim_p, dim_obs, state_p, ostate)
...
!!
  SUBROUTINE init_dim_obs_l_EN4_tho(domain_p, step, dim_obs, dim_obs_l)
...
!!
  SUBROUTINE localize_covar_EN4_tho(dim_p, dim_obs, HP_p, HPH, coords_p)
...
! *** Arguments ***
    ...
    WRITE (*,*) 'TEMPLATE init_EN4_tho_pdafomi_TEMPLATE.F90: Apply covariance localization'
...
! *************************************
  ...
  END SUBROUTINE localize_covar_EN4_tho

END MODULE obs_EN4_tho_pdafomi
